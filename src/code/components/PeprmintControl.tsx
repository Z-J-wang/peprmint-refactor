  ...
  return (
    ...
    >
      ...
      <Form.Group controlId="formPdbId" as={Row}>
        <Form.Label className="pr-0" column sm={colwidth}>
          {' '}
          Entry ID
        </Form.Label>
        ...
        <Col className="px-0">
          <Form.Text muted>
            ...
            <Popover
              ...
              content={
                <div>
                  7-character id, data source:{' '}
                  <a className="text-primary" href="https://www.cathdb.info/">
                    CATH
                  </a>
                  , e.g. 2da0A00
                </div>
      ...
      <Form.Group controlId="formInputPdb" as={Row}>
        <Form.Label column sm={colwidth} className="pt-0 pr-0 text-center">
          Or{' '}
        </Form.Label>
        <Col sm={9} className="pl-0">
          <Upload
            name="pdbFile"
            accept=".pdb, .ent, .cif, .mcif, .ply"
            maxCount={1}
            //  onChange={onChange}
      ...
      <Alert show={errorMessage != ''} variant="danger" onClick={() => setErrorMessage('')} dismissible>
        {errorMessage}
      </Alert>
...
function ControlArea({
  ...
  const protrusionPopoverContent = (
    <div style={{ width: 300 }}>
      ...
      <span className="text-muted">
        NOTE: the <b>C-β atom</b> of the residue is used as the representative in the visualisation.{' '}
      </span>
      <br />
      Read more: <b>Ref. 1</b>
    </div>
  ...
  const neighborPopoverContent = (
    <div style={{ width: 300 }}>Residues within a distance of 10Å to the hydrophobic protrusions</div>
  )
  ...
  const treeDataProtrusion = [
    {
      title: (
        <div>
          {' '}
          Show protrusions{' '}
          <Popover placement="topLeft" content={protrusionPopoverContent}>
            {' '}
            <QuestionCircleOutlined className="align-middle" />{' '}
          </Popover>{' '}
        </div>
      ...
      children: [
        {
          title: (
            <span>
              {' '}
              <span style={{ color: 'orange' }}> hydrophobic </span> protrusions{' '}
              <Popover placement="topLeft" content={hydroProtrusionPopoverContent}>
                {' '}
                <QuestionCircleOutlined className="align-middle" />{' '}
              </Popover>
            </span>
          ...
          children: [
            {
              title: (
                <span>
                  {' '}
                  co-insertable pairs{' '}
                  <Popover placement="topLeft" content={coinsterablePopoverContent}>
                    {' '}
                    <QuestionCircleOutlined className="align-middle" />{' '}
                  </Popover>{' '}
                </span>
            ...
            },
            {
              title: (
                <span>
                  select neighbour residues{' '}
                  <Popover placement="topLeft" content={neighborPopoverContent}>
                    {' '}
                    <QuestionCircleOutlined className="align-middle" />{' '}
                  </Popover>
                </span>
        ...
        },
        {
          title: (
            <span>
              {' '}
              all <span style={{ color: 'gray' }}> C-α, C-β </span> atoms{' '}
            </span>
          ),
          key: '0-0-1',
          selectable: false
        },
        {
          title: (
            <span>
              {' '}
              <span style={{ color: 'orange' }}> hydrophobic </span> <span style={{ color: 'gray' }}> C-α, C-β </span>{' '}
              atoms{' '}
            </span>
          ),
          key: '0-0-2',
  ...
  const treeDataConvexHull = [
    {
      title: (
        <span>
          Show convex hull{' '}
          <Popover placement="topLeft" content={convexHullPopoverContent}>
            {' '}
            <QuestionCircleOutlined className="align-middle" />{' '}
          </Popover>{' '}
        </span>
  ...
  const selectionPopoverContent = (
    <div style={{ width: 300 }}>
      {' '}
      Click the icon <IconButton svg={SelectionModeSvg} onClick={() => {}} /> on the viewer to enable the{' '}
      <a
        onClick={(e) => e.stopPropagation()}
        className="text-primary"
        href="https://molstar.org/viewer-docs/making-selections/"
      >
        selection mode
      </a>
      , then make a selection of your interested elements (residues, chains etc.), where the new calculation will be
      performed.
    </div>
  )
  ...
  const treeDataRecalculate = [
    {
      title: (
        <div>
          Use my current selection
          <Popover placement="topLeft" content={selectionPopoverContent}>
            {' '}
  ...
  const onCheckPortrusion = async (checkedKeys: any, info: any) => {
    ...
    const checkedKeysValue = checkedKeys as { checked: Key[]; halfChecked: Key[] }
    ...
    if (checkedKey === '0-0') {
      ...
      } else if (checkedKey === '0-0-0-1') {
        // highlight neighbors by selecting
  ...
  const onExpandConvexHull = (expandedKeys: React.Key[], info: any) => {
    if (info.expanded) {
      if (parentRef.current) {
        const parentWidth = (parentRef.current! as HTMLDivElement).offsetWidth
        setSliderWidth(`${parentWidth / 2}px`)
      }
    }
  }

  const emptySelectionErrorMsg = (
    <span>
      Current selection is <b>empty</b>. <br />
      Click the icon <IconButton svg={SelectionModeSvg} onClick={() => {}} /> on the viewer to start to select.
    </span>
  )
  ...
  return (
    <Container className="my-3 p-3 bg-light border">
      <div className="h5 mb-3 border-black border-bottom" ref={parentRef}>
        {' '}
        Basic settings{' '}
      </div>
      ...
      <Form.Text muted>
        Click the icons on the viewer plugin to explore more advanced features from{' '}
        <a className="text-primary" href={'https://molstar.org/viewer-docs/'}>
          {' '}
          Mol*{' '}
        </a>
      </Form.Text>
...
function NeighborInfoTable({ neighborDataSource }: any) {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'resName',
      render: (res: string) =>
        HYDROPHOBICS.includes(res.toUpperCase()) ? (
    ...
    },
    {
      ...
      render: (neighborStr: string) => {
        const resList = neighborStr.split(' ')
        const chainSet = new Set<string>()
        for (const res of resList) {
          const chain = res.split('|')[0]
          chainSet.add(chain)
        }
        const chainList = Array.from(chainSet.values())
        const newResList = []
        for (let i = 0; i < chainList.length; i++) {
          const chain = chainList[i]
          const oneChain = resList
            .filter((res) => res.split('|')[0] == chain)
            .map((res, i) => {
              const resName = res.split('|')[1]
