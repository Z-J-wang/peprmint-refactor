...
export const RES_COLORS = new Map(
  Object.entries({
    LEU: '#33FF00',
    ILE: '#66FF00',
    CYS: '#FFFF00',
...
export function ModuleCard(props: {
  ...
  const cover = (
    <span className="border-bottom border-primary">
      <Image src={props.imgSrc} preview={false} />{' '}
    </span>
  )
...
export function PageHeader(props: { headerList: PageHeaders[]; title: React.ReactNode; subtitle: React.ReactNode }) {
  ...
  const homeHeader =
    props.headerList[0] == PageHeaders.Home ? (
      <Breadcrumb.Item>
        {' '}
        <span>
          {' '}
          <Link className="text-primary" to="/">
            <HomeOutlined className="align-middle" /> PePrMInt{' '}
          </Link>
        </span>
      </Breadcrumb.Item>
    ) : (
  ...
  const pepr2visHeader = (
    <Breadcrumb.Item>
      {' '}
      <span>
        {' '}
        <EyeOutlined className="align-middle" /> PePr<sup>2</sup>Vis
      </span>{' '}
    </Breadcrumb.Item>
  )
  const pepr2dsHeader = (
    <Breadcrumb.Item>
      ...
      <span>
        {' '}
        <FileSearchOutlined className="align-middle" /> PePr<sup>2</sup>DS<sup className="text-muted">BETA</sup>
      </span>{' '}
    </Breadcrumb.Item>
  )

  const childHeader =
    props.headerList.length > 1 ? props.headerList[1] == PageHeaders.Pepr2vis ? pepr2visHeader : pepr2dsHeader : <> </>
  ...
  return (
    <>
      ...
      <Row className="mb-4 px-3">
        <h2>
          {' '}
          {props.title} <span className="text-muted font-weight-light"> {props.subtitle} </span>{' '}
        </h2>
      </Row>
    </>
  )
  ...
  render() {
    return (
      <Container className="my-3 p-3 bg-light">
        ...
        <ol>
          <ReferenceItem
            authors={'Fuglebakk E, Reuter N (2018) '}
...
function ReferenceItem(props: {
  ...
  return (
    <li>
      ...
      <a className="text-primary" href={props.url}>
        {' '}
        {props.title}{' '}
      </a>
