...
function importAllDatasets(r: __WebpackModuleApi.RequireContext) {
  ...
  r.keys().map((fileName: string) => {
    datasets[fileName] = r(fileName).default
  })
  return datasets
}
...
const renderCustomizedLabel = ({ cx, cy, midAngle, innerRadius, outerRadius, percent, fill, value, name }: any) => {
      ...
      <Card
        ...
        extra={
          <Radio.Group
            ...
            defaultValue={props.chartType ? props.chartType : 'pie'}
          >
            ...
            <Radio.Button value="bar">
              {' '}
              <BarChartOutlined className="align-middle" />{' '}
            </Radio.Button>
          </Radio.Group>
        }
        bordered={false}
...
export function Pepr2ds() {
  ...
  const calculateCompData = (colDataIndex: string, tableData: any) => {
    const compData = new Map<string, number>()
  ...
  const groupData = (
    ...
    const selectedData = tableData
      .map((record) => record[colDataIndex] && parseNumber(record[colDataIndex]))
    ...
    const dataGroups: number[][] = Array.from(Array(groupCount), (_) => [])
    for (const n of selectedData) {
      const groupIndex = Math.floor((n - dataMin) / step)
  ...
  useEffect(() => {
    ...
    const neighborResComp = new Map<string, number>(RESIDUES.map((r) => [r, 0]))
    for (const record of currentTableData) {
      if (record.nbl) {
        ...
        for (const resName of neighborResNames) {
          neighborResComp.set(resName, (neighborResComp.get(resName) || 0) + 1)
        }
      }
    }
    setNeighborResCompData(
      Array.from(neighborResComp, ([k, v]) => ({ name: k, value: v })).filter((e: any) => e.value != 0)
    )
  ...
  const trueFalseRender = (b: any) =>
    b && b.toLowerCase() == 'true' ? <CheckCircleTwoTone twoToneColor="#52c41a" /> : <> - </>

  const trueFalseFilter = [
  ...
  const defaultColumns = [
    ...
    },
    {
      ...
      width: 55,
      render: (pdbid: any) =>
        validPdbID(pdbid) ? <a href={'https://www.ebi.ac.uk/pdbe/entry/pdb/' + pdbid}> {pdbid} </a> : <>{pdbid}</>,
    ...
    },
    {
      title: (
        <Tooltip
          title={
            <>
              <b className="text-primary">I</b>nterfacial <b className="text-primary">B</b>inding{' '}
              <b className="text-primary">S</b>ite
            </>
          }
        >
          IBS
      ...
      dataIndex: 'ibs',
      width: 50,
      render: trueFalseRender,
      filters: trueFalseFilter,
      ...
      children: [
        {
          title: (
            <Tooltip
              title={
                <span>
                  convex hull <b className="text-primary">V</b>ertex
                </span>
              }
            >
              V
...

        {
          title: (
            <Tooltip
              title={
                <span>
                  <b className="text-primary">P</b>rotrusion
                </span>
              }
            >
              P
...

        {
          title: (
            <Tooltip
              title={
                <span>
                  <b className="text-primary">H</b>ydrophobic protrusion
                </span>
              }
            >
              H
...

        {
          title: (
            <Tooltip
              title={
                <span>
                  <b className="text-primary">C</b>o-insertable
                </span>
              }
            >
              C
        ...
        },
        {
          title: (
            <Tooltip
              title={
                <span>
                  whether residue is <b className="text-primary">E</b>xposed (RSA &gt; 20%) or not (RSA &lt;= 20%)
                </span>
              }
            >
              E
        ...
        },
        {
          title: (
            <Tooltip
              title={
                <span>
                  protein <b className="text-primary">D</b>ensity
                </span>
              }
            >
              D
          ...
          dataIndex: 'den',
          width: 40,
          render: (v: string) => (v && parseInt(v) > 0 ? v : <>-</>)
        },
        {
          title: 'neighbor residue list',
          dataIndex: 'nbl',
          width: 120,
          ellipsis: { showTitle: false },
          render: (
            nbl: string // customize the tooltip
          ) => (
            <Tooltip placement="topLeft" title={nbl.replace(/;/g, ' ')}>
              {' '}
              {nbl}{' '}
            </Tooltip>
          )
  ...
  const optionalColumns = [
...

    {
      ...
      dataIndex: 'ss',
      width: 70,
      filters: ['H', 'E', 'C'].map((ss) => {
        return { text: ss, value: ss }
      }),
...

    {
      ...
      dataIndex: 'uacc',
      width: 100,
      render: (uacc: any) => (uacc ? <a href={'https://www.uniprot.org/uniprot/' + uacc}> {uacc} </a> : <>{uacc}</>)
    },
    { title: 'Uniprot ID', dataIndex: 'uid', width: 120 },

    {
      title: 'Uniprot cluster id',
      children: [
  ...
  const onDownloadTableData = () => {
    ...
    for (const col of columns) {
      if (col.children) {
        selectedColumns = [
          ...selectedColumns,
          ...col.children.map((child: { title: any; dataIndex: any }) =>
    ...
    const downloadData = currentTableData.map((row) =>
      Object.assign(
        {},
        selectedColumns.map((col) => row[col.dataIndex])
      )
    )
    ...
    Papa.parse(file, {
      ...
      complete: function (results) {
        ...
        if (validDataHeaders.length == 0) {
          message.error(
            <>
              No valid data parsed from {file.name} <br /> Please check the format requirement for the data file{' '}
              <a href="https://github.com/reuter-group/peprmint-web/blob/main/web-client/src/datasets/README.md">
                defined here
              </a>
              .
            </>,
        ...
        newData.forEach((data) => {
          const tempData = { ...dataTemplate, data } as Object
      ...
      error: function (error, file) {
        message.error(
          <>
            This file {file.name} can not be parsed correctly. Please check if its format is valid as
            <a href="https://github.com/reuter-group/peprmint-web/blob/main/web-client/src/datasets/README.md">
              defined here
            </a>
            .
  ...
  return (
    <Container>
      <PageHeader
        headerList={[PageHeaders.Home, PageHeaders.Pepr2ds]}
        title={title}
      ...
      <Container className="mb-4 p-3 ">
        <Row className="mb-3 mx-3 justify-content-center">
          <h5>
            {' '}
            PePr<sup>2</sup>DS is a dataset of <b> interfacial binding sites</b>(IBS) collected from{' '}
            {Statistics.domainsList.length} protein domains.
          </h5>
          <p className="font-weight-light">
            {' '}
            (add short description here) The IBS are analyzed using the model for hydrophobic protrusions ...
        ...
        <Row className="mb-2 justify-content-center">
                  ...
                  <Popover
                    ...
                    content={
                      <>
                        {' '}
                        description here...{' '}
                        <a className="text-primary" href="https://www.ebi.ac.uk/pdbe/">
                          link example
                        </a>{' '}
                      </>
                    }
          ...
          <Col md={2} className="bg-light mx-4 py-2 border">
            ...
            <Statistic
              title={
                <>
                  Protein domains{' '}
                  <Popover placement="topLeft" content={Statistics.domainsList.toString()}>
                    <QuestionCircleOutlined className="align-middle" />{' '}
                  </Popover>
                </>
              }
          ...
          <Col md={2} className="bg-light mx-4 py-2 border">
            {' '}
            <Statistic title="Whole dataset" value="25.9 MB" />
            <small>
              <a className="text-muted" href={Statistics.downloadLink}>
            ...
            <BCard.Body>
              <Row className="my-4">
                ...
                <Col md={4}>
                  ...
                  >
                    <Button icon={<UploadOutlined />} className="border-primary">
                      Load my own dataset (.csv)
                    </Button>{' '}
                    &nbsp;
                    <Popover
                      placement="topLeft"
                      content={
                        <>
                          Optional, <b>one .csv</b> file that meets PePr<sup>2</sup>DS'{' '}
                          <a
              ...
              <Table
                ...
                pagination={{
                  ...
                  showTotal: (total) => (
                    <span>
                      {' '}
                      Total <b>{total}</b> rows in the table,{' '}
                    </span>
                  ),
                ...
                footer={() => (
                  <>
                    {' '}
                    For details of each column, please{' '}
                    <a
                      className="text-primary"
                      href="https://github.com/reuter-group/peprmint-web/blob/main/web-client/src/datasets/README.md"
                    >
              ...
              <Row className="justify-content-end">
                <Popconfirm
                  ...
                  title={
                    <>
                      <p>
                        <span className="text-muted">Note 1:</span> unselected optional columns will NOT be downloaded.
                      </p>
                      <p>
                        <span className="text-muted">Note 2:</span> choose which <b>table headers</b> to use:{' '}
                      </p>
                      <Radio.Group onChange={(e) => setDownloadLongHeaders(e.target.value)} value={downloadLongHeaders}>
                        <Space direction="vertical">
                          ...
                          <Radio value={2}>
                            short headers so that the .csv file can be re-loaded by PePr<sup>2</sup>DS
                          </Radio>
                        </Space>
                      </Radio.Group>{' '}
                    </>
                ...
                >
                  <BButton type="primary" className="mr-3">
                    <DownloadOutlined /> Selected dataset (.csv){' '}
                  </BButton>
                </Popconfirm>
              </Row>
            </BCard.Body>
            ...
            <BCard.Body>
              <Row>
                <ul>
                  <li>
                    {' '}
                    <b>{new Set(currentTableData.map((d) => d.pdb)).size}</b> unique PDB IDs,{' '}
                    <b>{new Set(currentTableData.map((d) => d.cath)).size}</b> unique CATH IDs{' '}
                  </li>
                  <li>
                    {' '}
                    <b>{currentTableData.length}</b> residues, including{' '}
                    <b>
                      {
                        currentTableData.filter(
                          (d) => d.hypro && d.hypro.toLowerCase() == 'true' && d.ibs && d.ibs.toLowerCase() == 'true'
                        ).length
                      }
                    </b>{' '}
                    residues which are both <b>H</b>ydrophobic protrusions and at <b>IBS</b>{' '}
                  </li>
                </ul>
              </Row>
              <Row className="my-4 mx-2">
                ...
                {resCompData.length > 0 && (
                  <ChartCard
                    cardTitle="Residue Composition"
                    chartData={resCompData}
                    chartNote={
                      <>
                        Total: <b>{currentTableData.length}</b> residues{' '}
                      </>
                    }
                  />
                )}
                ...
                {neighborResCompData.length > 0 && (
                  <ChartCard
                    cardTitle="Protrusion Neighbor Residue Composition"
                    chartData={neighborResCompData}
                    chartNote={
                      <>
                        Total: <b>{neighborResCompData.reduce((acc, data) => acc + data.value, 0)}</b> neighbor residues
                      </>
                    }
                  />
                )}
                  ...
                  <li>
                    ...
                    >
                      {[
                        ...
                        <Option value={col.dataIndex} key={i}>
                          {' '}
                          {col.name}{' '}
                        </Option>
                      ))}
              ...
              <Row className="my-4 mx-2">
                {proBloVis && currentTableData.length > 0 && (
                  <ChartCard
                    cardTitle="Protein Block"
                    chartData={proBloCompData}
                    chartSize="small"
                    chartType="bar"
                    chartNote={
                      <>
                        Total: <b>{proBloCompData.reduce((acc, data) => acc + data.value, 0)}</b> protein blocks{' '}
                      </>
                    }
                  />
                )}

                {proDenVis && currentTableData.length > 0 && (
                  <ChartCard
                    cardTitle="Protein Density"
                    chartData={proDenCompData}
                    chartSize="small"
                    chartType="bar"
                    chartNote={
                      <>
                        Total: <b>{proDenCompData.reduce((acc, data) => acc + data.value, 0)}</b> residues (with density
                        &gt; <b>{LOW_DENSITY_THRESHOLD}</b>)
                      </>
                    }
                  />
                )}

                {ssVis && currentTableData.length > 0 && (
                  <ChartCard
                    cardTitle="Secondary Structure"
                    chartData={ssCompData}
                    chartSize="small"
                    chartNote={
                      <>
                        Total: <b>{ssCompData.reduce((acc, data) => acc + data.value, 0)}</b> secondary structures{' '}
                      </>
                    }
                  />
                )}
