...
function getConvexHullMesh(data: ConvexHullData, props: ConvexHullProps, mesh?: Mesh) {
  ...
  if (data.showEdges) {
    for (let j = 0; j < data.edgePairs.length; j += 2) {
      ...
      const start = data.edgePairs[j],
        end = data.edgePairs[j + 1]
...
const SphereVisuals = {
  mesh: (ctx: RepresentationContext, getParams: RepresentationParamsGetter<ConvexHullData, ConvexHullParams>) =>
...
function ConvexHullRepresentation(
...
export const CreateConvexHull = CreateTransformer({
  ...
  apply({ a, params }, plugin: PluginContext) {
    ...
    return Task.create('Custom Convex Hull', async (ctx) => {
      ...
      return new SO.Shape.Representation3D({ repr, sourceData: params }, { label: `Convex Hull` })
    })
  },
